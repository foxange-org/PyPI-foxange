...
- `version()->str`
  ...
  注意,这个函数在0.3.3以后(不包含0.3.3)都不输出`version : {version}` 文字了
...
- `pow(number1: float, number2: float) -> float`
  ...
  **注意**：会覆盖 Python 内置 `pow()`，如需内置请用 `__builtins__.pow`。
  **代码示例**：
...
- `sum(*values)`
  增强求和函数，支持数字和列表混合。
  **注意**：会覆盖内置 `sum()`，如需内置请用 `__builtins__.sum`。
  **代码示例**：
...
- `collect_input(*value)->list`
  ...
  **注意** : 如果`*value`是一个或多个列表,`collect_input`会直接类成似于`input([1,2,3,4])`差不多的效果

- `sanitize_input(text:str,*strings)->str`

  这是一个删除`text`字符串中所有的`*strings`子字符串
...
- `numeric_input(text:str,min:int,max:int,netvalid:Any=None)->Any`
  ...
  这是一个判断`input(text)`字符串的长度是否**大于等于`min`**或**小于等于`max`**
...
- `remove(value:list,condition=None)->list`
  ...
  删除满足条件的值,这个`condition`**必须是`None`或`lambda`表达式!!!**
...
- `ensure_dir(path: str) -> None`
  创建目录（如果不存在），相当于 `os.makedirs(path, exist_ok=True)`。
  **代码示例**：
...
`foxangePyPI@outlook.com`发送问题

发送格式: `函数`   `问题`    `你的代号` 我们会在这个文件里记录你的名字,从而感谢你对foxange第三方库的贡献和支持!

**foxange** 第三方库开源github链接 - [foxange-org/PyPI-foxange]([foxange-org/PyPI-foxange： foxange的python第三方库](https://github.com/foxange-org/PyPI-foxange))
